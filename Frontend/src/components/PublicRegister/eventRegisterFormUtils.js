...
export const buildRegistrationErrors = ({
  ...
  if (isDeclarationRequired(regConfig) && !formData.acceptedTerms) {
  ...
  if (show("dob") && formData.dateOfBirth) {
    const birthDate = new Date(formData.dateOfBirth);
    const age = new Date().getFullYear() - birthDate.getFullYear();
    if (age < 18) errors.dateOfBirth = "You must be at least 18 years old";
  }
  if (show("mobile") && formData.phone && formData.phone.length !== 10) {
  ...
  if (isEmailOtpEnabled(regConfig) && !emailOtpVerified) {
